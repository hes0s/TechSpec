...
export class Body {
  ...
  updateCart() {
    this.localS.cart.subscribe((cartItems) => {
      ...
      this.cartTotal = cartItems.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
  ...
  submitOrder() {
    ...
    this.dataService.addOrder(order).then(() => {
  ...
  ngOnInit() {
    ...
    this.auth.getStatus().subscribe((user) => {
      if (user) {
        this.loggedIn = true;
        this.dataService.getUserOrders(user.email!).subscribe((orders) => {
          this.userOrders = orders;
        });
      } else {
        this.loggedIn = false;
